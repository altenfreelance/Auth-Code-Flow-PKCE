...
const useAuthCodeFlowPKCE = (clientId, audience, authorizeEndpoint, tokenEndpoint) => {
    ...
    const prevJwt = useRef();
    const [refreshToken, setRefreshToken] = useState(null)
    const [jwtExpiration, setJwtExpiration] = useState(null)
    
    const [callbackAfterRefresh, setCallbackAfterRefresh] = useState(null)
    ...
    useEffect(() =>{
        if (callbackAfterRefresh){
          callbackAfterRefresh(jwt)

          setCallbackAfterRefresh(null)
        }
    ...
    function refreshJwtAndDoCallback(callback) {
        ...
        axios.request(refreshOptions)
          .then(function (response) {
            ...
            setCallbackAfterRefresh(() => callback)
            setJwt(access_token)
            setRefreshToken(refresh_token)            
            setJwtExpiration(getJwtExpiration(access_token))
    ...
    return [jwt, jwtExpiration, refreshJwtAndDoCallback]
